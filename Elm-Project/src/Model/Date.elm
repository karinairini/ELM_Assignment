...
monthsBetween dA dB =
    case (month dA, month dB) of
        (Just month1, Just month2) -> Maybe.map (\diff -> abs diff) <| Just ((12 * year dA + monthToInt month1) - (12 * year dB + monthToInt month2))
        (Nothing, Nothing) -> Just <| (abs((12 * (year dA)) - (12 * (year dB))))
        _ -> Nothing
...
compare (Date d1) (Date d2) =
    let
        compareYears = Basics.compare d1.year d2.year
        compareMaybeMonths month1 month2 =
            case (month1, month2) of
                (Just month1_1, Just month2_2) -> compareMonth month1_1 month2_2
                (Just _, Nothing) -> LT
                (Nothing, Just _) -> GT
                _ -> EQ
    in
    chainCompare (compareMaybeMonths d1.month d2.month) compareYears
...
view (Date d) =
    div []
        ...
        , text " "
        , Maybe.withDefault (text "") (Maybe.map (text << monthToString) d.month)
        ]
